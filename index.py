...
def importacao_espumantes(ano: int = Query(2024)):
    ...
    return {f"Importacao espumantes - {ano}": dados}
...
def importacao_uvas_passas(ano: int = Query(2024)):
    ...
    return {f"Importacao uvas passas - {ano}": dados}
...
def exportacao_vinho_de_mesa(ano: int = Query(2024)):
    s = Embrapa()
    dados = s.exportacao_vinho_de_mesa(ano)
    return {f"Exportacao vinho de mesa - {ano}": dados}
...
def exportacao_espumantes(ano: int = Query(2024)):
    ...
    return {f"Exportacao espumantes - {ano}": dados}
...
def exportacao_uvas_frescas(ano: int = Query(2024)):
    ...
    return {f"Exportacao uvas frescas - {ano}": dados}
